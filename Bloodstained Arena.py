...
###TEXT
class colour:   #Colours only display properly when run directly from the exe, they bug out in shell
...
for x in range (0, len(cards)):
    ...
    card[6] = int(card[6])  #Usually a dmg value
...
ai_enabled = 0   #When the ability to play vs AI is added, this can be changed via an input() command
...
###CORE FUNCTIONS
def clear():    #Clear the screen - only works if run directly from the exe, does NOT work in python shell
    if name == "nt":
        _ = system("cls")
...
def play_round():
    ...
    #I don't actually know if these three NEED to be global but I'm making them global just in case, to prevent any potential errors
    global round_count
    ...
    if p1_health > 0 and p2_health <= 0:    #p1 kills p2
        ...
        if p1_health > p2_health:
            p2_lives -= lives_to_lose
            print("Player 1 wins this round by having more health than player 2 after offensive options were exhausted!")
            p1_defeats_p2()
        elif p1_health < p2_health:
            p1_lives -= lives_to_lose
            print("Player 2 wins this round by having more health than player 1 after offensive options were exhausted!")
            p2_defeats_p1()
        elif p1_health == p2_health:
...
def p1_defeats_p2():    #Just so I don't have to repeat this code several times
    if p2_lives == 1:
        input("Player 2 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+"1"+colour.END+" life remaining. ")
    if p2_lives <= 0:
        input("Player 2 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+"NO"+colour.END+" lives remaining! ")
    else:
        input("Player 2 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+str(p2_lives)+colour.END+" lives remaining. ")

def p2_defeats_p1():    #Just so I don't have to repeat this code several times
    if p1_lives == 1:
        input("Player 1 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+"1"+colour.END+" life remaining. ")
    if p1_lives <= 0:
        input("Player 1 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+"NO"+colour.END+" lives remaining! ")
    else:
        input("Player 1 has lost "+colour.RED+str(lives_to_lose)+colour.END+" lives and now has "+colour.RED+str(p1_lives)+colour.END+" lives remaining. ")
...
def p1_shop():  #Player 1's shop phase
    ...
    #If there are no attack cards in player's hand and no attack cards in the draw, replace the 4th card with quick slash
    if not ("Melee" in [x[0] for x in p1_hand] or "Ranged" in [x[0] for x in p1_hand] or any(x[0] == "Item" and x[5] > 0 for x in p1_hand) or any(x[0] == "Magic" and x[5] != "Healing" for x in p1_hand)) and not ("Melee" in [x[0] for x in drawn_cards] or "Ranged" in [x[0] for x in drawn_cards] or any(x[0] == "Item" and x[5] > 0 for x in drawn_cards) or any(x[0] == "Magic" and x[5] != "Healing" for x in drawn_cards)):
        drawn_cards.pop(3)
        drawn_cards.append(deck[0]) #ALWAYS make sure the first entry in Cards.txt is Quick Slash for this to work
...
def p1_printdraw(drawn_cards):
    ...
    if buy == "1" or buy == "2" or buy == "3" or buy == "4":
        ...
        if "Melee" in [x[0] for x in p1_hand] or "Ranged" in [x[0] for x in p1_hand] or any(x[0] == "Item" and x[5] > 0 for x in p1_hand) or any(x[0] == "Magic" and x[5] != "Healing" for x in p1_hand):
            input("Now ending player 1's shop phase. Player 2, please take the seat. ") #Show a different message when playing against AI
            return
        else:
            clear()
...
def p2_printdraw(drawn_cards):
    ...
    if buy == "1" or buy == "2" or buy == "3" or buy == "4":
        ...
        if "Melee" in [x[0] for x in p2_hand] or "Ranged" in [x[0] for x in p2_hand] or any(x[0] == "Item" and x[5] > 0 for x in p2_hand) or any(x[0] == "Magic" and x[5] != "Healing" for x in p2_hand):
            input("Now ending player 2's shop phase. Player 1, please take the seat. ") #Show a different message when playing against AI
            return
        else:
            clear()
            noattack = input("You are about to end your shop phase with no damage-dealing cards in your hand.\nUnless your opponent also has no damage-dealing cards, you are guaranteed to lose the next round. Are you sure you want to continue? (y/n) ")
...
def p1_battle():
    ...
    if (any(x[0] == "Melee" and x[5] > 0 for x in p1_hand) or "Ranged" in [x[0] for x in p1_hand] or any(x[0] == "Item" and x[5] > 0 for x in p1_hand) or any(x[0] == "Magic" and x[5] != "Healing" for x in p1_hand)) and p1_exhausted == 0:
        if p2_exhausted == 0:
            turn = input("Type the number of the card you wish to play. Keep in mind that Passive cards are not playable in rounds.\nTo forfeit this round, type \"f\". To end your turn and hand control to the other player, type \"n\". ")
        else:
            turn = input("Type the number of the card you wish to play. Keep in mind that Passive cards are not playable in rounds.\nTo forfeit this round, type \"f\". The other player has exhausted all their offensive options, so forfeiting now will result in victory this round as long as you have the highest health.\nTo end your turn and hand control to the other player, type \"n\". ")
        if turn == "1" or turn == "2" or turn == "3" or turn == "4" or turn == "5" or turn == "6" or turn == "7":
                    ...
                    if not p1_hand[turn][0] == "Item" and p1_hand[turn][3] > p1_energy:
                        ...
                        #Playing a Melee card to deal damage, uses round use limits rather than being removed on use
                            ...
                            if p1_hand[turn][5] > 0:
                                p1_hurts_p2(p1_hand[turn][6])
                                p1_energy -= p1_hand[turn][3]
                                p1_hand[turn][5] -= 1    #Uses remaining
                                if p1_hand[turn][5] == 1:
            ...
            input("Now ending player 1's turn. Player 2, please take the seat. ") #Show a different message when playing against AI
            return
        elif turn == "f":
            p1_exhausted = 1
            p1_did_forfeit = 1
        ...
        if p1_did_forfeit == 0:
            p1_exhausted = 1
            input("You have no usable damage-dealing cards remaining, and therefore your turn must be ended. ")
        else:
            input("You have forfeited this round and so cannot take any actions until the next round. ")
        return
...
def p2_battle():
    ...
    for x in range (0,len(p2_hand)):
        ...
        if p2_hand[x][0] == "Melee" or p2_hand[x][0] == "Ranged":
            ...
            print("\tDamage: "+colour.RED+str(p2_hand[x][6])+colour.END)
        elif p2_hand[x][0] == "Magic" and p1_hand[x][5] == "Healing":
            print("\tHealing: "+colour.GREEN+str(p1_hand[x][6])+colour.END)
        elif p2_hand[x][0] == "Item" and p2_hand[x][5] > 0:
            print("\tDamage: "+colour.RED+str(p2_hand[x][5])+colour.END)
        elif p2_hand[x][0] == "Item" and p2_hand[x][6] > 0:
    ...
    #Actual battle functions can only occur if the player has any usable damage-dealing cards remaining
    if (any(x[0] == "Melee" and x[5] > 0 for x in p2_hand) or "Ranged" in [x[0] for x in p2_hand] or any(x[0] == "Item" and x[5] > 0 for x in p2_hand) or any(x[0] == "Magic" and x[5] != "Healing" for x in p2_hand)) and p2_exhausted == 0:
        if p1_exhausted == 0:
            turn = input("Type the number of the card you wish to play. Keep in mind that Passive cards are not playable in rounds.\nTo forfeit this round, type \"f\". To end your turn and hand control to the other player, type \"n\". ")
        else:
            turn = input("Type the number of the card you wish to play. Keep in mind that Passive cards are not playable in rounds.\nTo forfeit this round, type \"f\". The other player has exhausted all their offensive options, so forfeiting now will result in victory this round as long as you have the highest health.\nTo end your turn and hand control to the other player, type \"n\". ")
        if turn == "1" or turn == "2" or turn == "3" or turn == "4" or turn == "5" or turn == "6" or turn == "7":
            ...
            input("Now ending player 2's turn. Player 1, please take the seat. ") #Show a different message when playing against AI
            return
        elif turn == "f":
            p2_exhausted = 1
            p2_did_forfeit = 1
...
input("Welcome to the digital version of the card game Bloodstained Arena. The functionality to play vs AI is not yet finished, so currently only hotseat multiplayer is available.\n")
clear()
